...
if(isset($_POST['signup_btn'])){
    ...
    if($stmt->num_rows() > 0){
        ...
        $stmt = $conn->prepare("INSERT INTO usuario (username,email,data_nascimento,telefone,senha) VALUES (?,?,?,?,?)");
        $stmt->bind_param("sssss",$username,$email,$data_nascimento,$telefone,$password);
        ...
        $stmt->execute();
        // talvez de erro $stmt->bind_result($id,$username,$email,$data_nascimento,$telefone,$image,$posts,$bio);
        $stmt->fetch();
