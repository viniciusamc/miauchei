...
include("../php/connection.php");
        ...
        if(empty($ids)){
               $error_message = "Você não deu Guard em nenhum post ainda!!!";
                ...
                $stmt = $conn->prepare("SELECT COUNT(*) as total_posts FROM posts_animais_perdidos WHERE id in ($ids_of_posts_you_guard)");
                $stmt->execute();
                ...
                $stmt = $conn->prepare("SELECT * FROM posts_animais_perdidos WHERE id in ($ids_of_posts_you_guard) ORDER BY id DESC LIMIT $offset,$total_posts_per_page"); 
                $stmt->execute();
...
?>
    ...
    <section class="feed">
      ...
      <?php if(!isset($posts)){ ?>
             ...
             <section class="mx-auto mt-5 alert alert-danger"><?php echo $error_message; ?>
             </section>
