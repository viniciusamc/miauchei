...
if(!isset($_SESSION['id'])){
    ...
    exit;
      ...
      if (isset($_POST['delete_conta_perfil_usuario'])) {
          $delete_id = $_SESSION['other_user_id'];
      ...
      if ($verificar AND $verificar2 AND $verificar3) {
        echo ('<script>window.alert("Deletado com sucesso"); window.location="home_adm.php"</script>');
      }else{
        echo('<script>window.alert("Falha na Deletação"); window.location="other_user_profile.php"</script>');
      }
